...
function users_listing($atts){ /* Front-End Display*/
	...
	for($i=0;$i<count($user_id);$i++){
		...
		if (in_array(ucfirst($userrole->roles[0]), $getroles)){ 
			...
			$url = get_author_posts_url( $user_id[$i] );
			$plugin_content = $plugin_content.'
			<div class="author col-xs-12 col-sm-3 col-md-3 '.$user_info->user_nicename.'">
				<div class="authorAvatar">
					<img src="'.$user_info->user_img.'">
					<div class="authorOverlay">
						<a href="'.$url.'">Read More</a>
					</div>
				</div>
				<h3 class="authorName"><a href="'.$url.'">'.$user_info->display_name.'</a></h3>
			</div>';
